...
def normalizar_telefono(telefono: str) -> str:
    # E.164 sin '+', sin espacios, sin 0 inicial. Corrige prefijos comunes.
    limpio = re.sub(r"\D", "", telefono or "")
    if limpio.startswith("00"):        # prefijo internacional 00 -> quitar
        limpio = limpio[2:]
    limpio = limpio.lstrip("0")        # ceros iniciales de operadora/typo
    if len(limpio) == 9 and limpio[0] in "679":  # movil/fijo ES sin prefijo -> 34
        limpio = "34" + limpio
    return limpio


async def _post_meta(payload: dict) -> bool:
    # Nunca rompe el flujo: si falta config o falla Meta, solo loguea.
...
async def enviar_whatsapp_template(telefono: str, template: str, params: list) -> bool:
    # Modo plantilla (requerido por Meta fuera de la ventana de 24h).
    return await _post_meta({
        "messaging_product": "whatsapp",
        "recipient_type": "individual",
        "to": normalizar_telefono(telefono),
...
def _plantilla(tipo: str, cita: dict) -> str:
    ...
    if tipo == "recordatorio":
        return (f"¡Hola {cita['client_name']}! Mañana te espero en +58 BarberStudio: "
                f"{cita['service_name']} a las {cita['start']} ({cita['date']}).\n"
                f"Tu código: {codigo}. Modifícala o cancélala (hasta 12h antes): {GESTIONAR_URL}")
    if tipo == "barbero":
        return (f"📋 Nueva cita: {cita['client_name']} ({cita['client_phone']}) — "
                f"{cita['service_name']}, {cita['date']} {cita['start']}–{cita['end']}. "
                f"Agenda: {ADMIN_URL}")
    return (f"¡Hola {cita['client_name']}! Tu cita en +58 BarberStudio está confirmada: "
...
async def generar_mensaje_gemini(tipo: str, cita: dict) -> str:
    # Mensaje personalizado con Gemini; fallback a plantilla estatica. El enlace siempre se garantiza.
    ...
    try:
        ...
        logger.error("Gemini error (%s); usando plantilla estatica", e)
        return fallback


_TPL_ENV = {
    "confirmación": "WA_TPL_CONFIRM", "confirmacion": "WA_TPL_CONFIRM",
    "barbero": "WA_TPL_BARBER", "recordatorio": "WA_TPL_REMINDER",
    "cancelacion": "WA_TPL_CANCEL", "cancelacion_barbero": "WA_TPL_CANCEL_BARBER",
    "modificacion": "WA_TPL_MOD", "modificacion_barbero": "WA_TPL_MOD_BARBER",
}
...
async def enviar_a(telefono: str, tipo: str, cita: dict) -> bool:
    ...
    tpl = os.environ.get(_TPL_ENV[tipo], "")
    if tpl:
...
async def notificar_nueva_cita(cita: dict) -> None:
    # Tras crear una cita: aviso al barbero (BUSINESS_WHATSAPP) y confirmacion al cliente.
    try:
        ...
        logger.error("notificar_nueva_cita error: %s", e)


async def notificar_cambio_cita(cita: dict, accion: str) -> None:
    # accion: "cancelacion" | "modificacion" — avisa al barbero y confirma al cliente.
    try:
        barbero = os.environ.get("BUSINESS_WHATSAPP", "")
        if barbero:
        ...
        logger.error("notificar_cambio_cita error: %s", e)


async def notificar_multi(citas: list) -> None:
    # 2-3 citas del mismo cliente: UN mensaje consolidado si hay plantillas (WA_TPL_*_2 / _3).
    n = len(citas)
    if n < 2 or n > 3:
        for c in citas:
    ...
    try:
        ...
        logger.error("notificar_multi error: %s", e)


async def probar_envio(telefono: str, plantilla: str = "", lang: str = "", params: list = None, texto: str = "") -> dict:
    # Diagnóstico: envía la plantilla configurada (o hello_world) y DEVUELVE la respuesta de Meta.
    token = os.environ.get("WHATSAPP_TOKEN", "")
    phone_id = os.environ.get("PHONE_NUMBER_ID", "")
    if not token or not phone_id:
    ...
    if texto:
        ...
        if tpl and tpl != "hello_world":
            template = {"name": tpl, "language": {"code": lang or os.environ.get("WHATSAPP_TEMPLATE_LANG", "es_ES")}}
            if params is None:
                params = ["Cliente Prueba", "Corte y Barba", "2026-06-16", "10:00", "abc12345"]
            if params:
                template["components"] = [{"type": "body", "parameters": [
                    {"type": "text", "text": str(p)[:1000]} for p in params
                ]}]
        else:
    ...
    try:
        ...
        if resp.is_success and not entregado:
            aviso = "Meta aceptó la petición pero NO encoló el mensaje (falta message_status=accepted): el texto libre fuera de la ventana 24h se descarta en silencio — usa una plantilla aprobada o haz que el cliente escriba primero."
        return {"ok": resp.is_success, "entregado": entregado, "aviso": aviso, "status": resp.status_code, "plantilla_usada": template["name"], "emisor": emisor, "meta_response": body}
    except Exception as e:
        return {"ok": False, "error": str(e)}


async def reenviar_respuesta_cliente(de_telefono: str, nombre: str, texto: str) -> bool:
    # Reenvía al barbero (BUSINESS_WHATSAPP) lo que un cliente escribe al número de la API.
    barbero = os.environ.get("BUSINESS_WHATSAPP", "")
    if not barbero:
    ...
    # Fallback texto libre: solo funciona si el barbero tiene ventana de 24h abierta con el número.
