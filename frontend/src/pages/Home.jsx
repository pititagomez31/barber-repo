...
import { Scissors, Clock, Star, MapPin, ChevronRight, Award } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#14141A] text-neutral-100 pb-24 md:pb-0" data-testid="home-page">
      ...
      {/* GESTIONAR CITA */}
      <section className="py-16 md:py-24 px-5 md:px-6 bg-[#17171B] border-t border-white/5" data-testid="home-manage-section">
        <div className="max-w-3xl mx-auto">
          <div className="mb-8 md:mb-10 text-center">
            <p className="tracking-overline uppercase text-[10px] sm:text-xs text-[#D4B77A] mb-3 md:mb-4">Tu reserva</p>
            <h2 className="font-display text-4xl md:text-6xl tracking-tight italic leading-[1.05]">¿Ya tienes<br />una cita?</h2>
            <p className="text-neutral-400 mt-4 text-sm md:text-base leading-relaxed">Modifícala o cancélala con tu código de 8 caracteres y tu teléfono. Cancelaciones hasta 12h antes.</p>
          </div>
          <ManageBooking id="gestionar" defaultOpen />
        </div>
      </section>
      ...
      <section id="galeria" className="py-16 md:py-24 px-5 md:px-6 bg-[#17171B]" data-testid="gallery-section">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 auto-rows-[170px] md:auto-rows-auto md:grid-rows-2 gap-3 md:gap-4 md:h-[520px]">
      ...
      <section id="sobre" className="py-16 md:py-24 px-5 md:px-6" data-testid="about-section">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8 md:gap-14 items-center">
          <div className="relative">
            <div className="aspect-[4/5] rounded-lg border border-[#2A2A32] overflow-hidden bg-[#1A1A1E]">
              <img src="https://customer-assets-jai6qajn.emergentagent.net/job_barber-v2-preview/artifacts/ww6k42kp_223d1a0a6feb642aee8e7a416c179972.jpg" alt="Heber, barbero de +58 BarberStudio" className="w-full h-full object-cover object-top" />
            </div>
          ...
          <div>
            ...
            <p className="text-neutral-400 mt-5 md:mt-6 leading-relaxed text-sm md:text-base">
              Soy <span className="text-[#D4B77A]">Heber</span>. Formado entre Venezuela y las Islas Canarias, mi obsesión es que salgas por la puerta con un corte que te haga sentir tú al 200%. Trabajo con cita previa para dedicarte el tiempo que mereces, sin esperas. Y si algún día voy unos minutos justos —diez o quince como mucho—, tenme paciencia: el mismo cuidado que le doy a quien está en la silla te lo daré a ti.
            </p>
            <ul className="mt-5 md:mt-6 space-y-2 text-sm text-neutral-300">
              <li>· Corte a tijera y máquina</li>
      ...
      {/* RESEÑAS GOOGLE */}
      <section id="resenas" className="py-16 md:py-24 px-5 md:px-6" data-testid="reviews-cta-section">
        <div className="max-w-3xl mx-auto text-center">
          <p className="tracking-overline uppercase text-[10px] sm:text-xs text-[#D4B77A] mb-5">Google Reviews</p>
          <div className="flex justify-center gap-1.5 mb-5">
            {[...Array(5)].map((_, i) => <Star key={i} className="h-7 w-7 md:h-9 md:w-9 fill-[#D4B77A] text-[#D4B77A]" />)}
          </div>
          <p className="font-display italic text-5xl md:text-6xl text-[#D4B77A]" data-testid="reviews-rating">
            {business.rating || "5,0"}<span className="text-2xl md:text-3xl text-neutral-500">/5</span>
          </p>
          {business.reviews_count ? (
            <p className="text-neutral-500 text-sm mt-2" data-testid="reviews-count">{business.reviews_count} reseñas en Google</p>
          ) : null}
          <h2 className="font-display text-3xl md:text-5xl tracking-tight italic mt-8 mb-4">¿Te gustó el resultado?<br />Cuéntalo en Google.</h2>
          <p className="text-neutral-400 text-sm md:text-base mb-8 max-w-xl mx-auto leading-relaxed">
            Tu reseña de 5 estrellas ayuda a que más gente confíe en la barbería. Solo te lleva un minuto.
          </p>
          <a href={business.reviews_url || "https://g.page/r/Ccfra4SBDdjCEBM/review"} target="_blank" rel="noreferrer" data-testid="leave-review-btn">
            <Button className="h-12 px-8 bg-[#D4B77A] hover:bg-[#C2A366] text-[#14141A] font-semibold btn-shine">Déjanos tu reseña de 5 estrellas</Button>
          </a>
        </div>
      </section>
